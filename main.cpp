...
void buyMenu(IngredientList& IL, MoneyBox& MB); // 재고관리 메뉴
...
void sellMenu(ProductList& PL, ProductTotal& PT, IngredientList& IL, MoneyBox& MB) {
    SellProduct SP;
    const char *menuList[] = { "아메리카노","카페라떼","카페모카","홍차","녹차","딸기에이드","레몬에이드","딸기스무디","초코스무디" };
    int menuCnt = sizeof(menuList) / sizeof(menuList[0]);
    int menuNum;
    int sellCnt;
    int sellMoney;
    ...
    while (true) {
        ...
        sellMoney = SP.ProductCntIncreament(PL, sellCnt, menuNum);
        SP.IngredientCntDecreament(IL, menuNum, sellCnt);
        SP.MoneyIncreament(MB, sellMoney);
        PT.addSellCount(menuNum, sellCnt);
    }
...
void buyMenu(IngredientList& IL, MoneyBox& MB) {
    ...
    while (true) {
        ...
        if (menuNum == menuCnt) { break; }
        buyMoney = BI.ProductCntDecreament(IL, cnt, menuNum);
        BI.IngredientCntIncreament(IL, cnt, menuNum);
        BI.MoneyIncreament(MB, buyMoney);
    }
...
void viewMenu(MoneyBox& MB) {
    const char *menuList[] = { "매상 내역","목표 매출 출력" };
    int menuCnt = sizeof(menuList) / sizeof(menuList[0]);
    int menuNum;
    displayTitle("매상 관리");
    while (true) {
